...
def test_secure_data(make_access_token, make_flask_post_request):
    """Тест на проверку доступа с защищённой информации"""
    headers = {
        "Authorization": f"Bearer {make_access_token}"
    }
    rv1 = make_flask_post_request(path="secure", headers=headers)
    rv2 = make_flask_post_request(path="secure")
